...
<form method="POST" action="{{ $editing ? route('campaigns.update', $campaign) : route('campaigns.store') }}" class="space-y-6">
    ...
    <div class="page-card-soft">
        <p class="eyebrow">{{ __('Campaign identity') }}</p>
    ...
    <div class="grid gap-6 md:grid-cols-2">
        ...
        <div>
            ...
            <select id="game_system_id" name="game_system_id" class="form-surface mt-1 block w-full rounded-2xl border px-4 py-3 shadow-sm" required>
        ...
        <div class="md:col-span-2">
            ...
            <textarea id="synopsis" name="synopsis" rows="3" class="form-surface mt-1 block w-full rounded-[1.5rem] border px-4 py-3 shadow-sm" required>{{ old('synopsis', $campaign->synopsis ?? '') }}</textarea>
            <x-input-error class="mt-2" :messages="$errors->get('synopsis')" />
        </div>

        <div class="md:col-span-2">
            <x-input-label for="description" :value="__('Description')" />
            <textarea id="description" name="description" rows="5" class="form-surface mt-1 block w-full rounded-[1.5rem] border px-4 py-3 shadow-sm">{{ old('description', $campaign->description ?? '') }}</textarea>
        </div>

        <div class="md:col-span-2">
            <x-input-label for="rules_summary" :value="__('House rules / summary')" />
            <textarea id="rules_summary" name="rules_summary" rows="4" class="form-surface mt-1 block w-full rounded-[1.5rem] border px-4 py-3 shadow-sm">{{ old('rules_summary', $campaign->rules_summary ?? '') }}</textarea>
        </div>
        ...
        <div>
            ...
            <select id="visibility" name="visibility" class="form-surface mt-1 block w-full rounded-2xl border px-4 py-3 shadow-sm" required>
        ...
        <div>
            ...
            <select id="status" name="status" class="form-surface mt-1 block w-full rounded-2xl border px-4 py-3 shadow-sm">
